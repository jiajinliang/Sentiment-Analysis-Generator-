...
class Model:
    ...
    def explain_coef(self):
        ...
        for counter in range(0, len(self.sentiment.train_labels)):
            if self.sentiment.train_labels[counter] == "FAKE":
    ...
    #             print("Number of ",word,"in POSITIVE reviews: ",num_p,"\tNumber of ",word,"in NEGATIVE reviews: ",num_n,"\n")
        ...
        dic = {'_Feature_':word_list, '_Coefficient_':coef_list, '_REAL_':num_p_list,
              '_FAKE_':num_n_list, '_TFIDF_':tfidf_list, '_Count_':count_list,
              '_Contribution_':contribution_list}
        df = pd.DataFrame(dic)
        df = df[['_Feature_','_Contribution_','_Count_','_TFIDF_','_Coefficient_','_REAL_','_FAKE_']]
    #     print(df)
        return df
    ...
    def bar_chart(self, cd, title,ylabel):
        ...
        l.sort()
        if len(l) > 10:
            words = [w for c,w in l[:5]] + [w for c,w in l[-5:]]
            coefs = [c for c,w in l[:5]] + [c for c,w in l[-5:]]
        else:
